...
public class IncenseProcessor implements IComponentProcessor {
	...
	public void setup(World level, IVariableProvider variables) {
		recipe = level.getRecipeManager().get(Identifier.tryParse(variables.get("recipe", level.getRegistryManager()).asString())).map(RecipeEntry::value).filter(recipe -> recipe.getType().equals(BWRecipeTypes.INCENSE_RECIPE_TYPE)).map(recipe -> (IncenseRecipe) recipe).orElseThrow(IllegalArgumentException::new);
	}

	@Override
	public IVariable process(World level, String key) {
		if (key.equals("header")) {
			return IVariable.from(recipe.effect.getName(), level.getRegistryManager());
		}
